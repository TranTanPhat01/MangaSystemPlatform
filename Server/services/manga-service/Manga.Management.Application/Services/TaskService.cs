...
public sealed class TaskService : ITaskService
{
    ...
    public async Task<Result<SubmissionResponse>> SubmitAsync(Guid id, SubmitTaskRequest request, Guid currentUserId, CancellationToken cancellationToken = default)
    {
        ...
        if (task.Status is not (DomainTaskStatus.InProgress or DomainTaskStatus.RevisionRequired)) return Result<SubmissionResponse>.Failure("Only in-progress or revision-requested tasks can be submitted.");
    ...
    public async Task<Result<TaskResponse>> ApproveAsync(Guid id, CancellationToken cancellationToken = default)
    {
        ...
        task.Status = DomainTaskStatus.Approved; task.UpdatedAt = DateTime.UtcNow;
        var submission = await GetLatestSubmissionAsync(id, cancellationToken); if (submission is not null) submission.Status = SubmissionStatus.Approved;
        await _eventBus.PublishAsync(new TaskApprovedEvent(Guid.NewGuid(), id, task.CreatedByUserId, DateTime.UtcNow), cancellationToken);
        await _unitOfWork.SaveChangesAsync(cancellationToken);
        return Result<TaskResponse>.Success(await ToResponseAsync(task, cancellationToken));
    }
    ...
    public async Task<Result<TaskResponse>> RequestRevisionAsync(Guid id, RequestRevisionRequest request, Guid currentUserId, CancellationToken cancellationToken = default)
    {
        ...
        var submission = await GetLatestSubmissionAsync(id, cancellationToken); if (submission is not null) submission.Status = SubmissionStatus.RevisionRequired;
    ...
    private async Task<Submission?> GetLatestSubmissionAsync(Guid taskId, CancellationToken cancellationToken) =>
        (await _repository.ListAsync<Submission>(submission => submission.TaskId == taskId, cancellationToken)).OrderByDescending(submission => submission.SubmittedAt).FirstOrDefault();
