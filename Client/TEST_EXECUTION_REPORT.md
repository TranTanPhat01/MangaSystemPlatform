...
| **FE-003** | Frontend full quality gate | P0 | ⚠️ **PASS WITH RISKS** | Next.js Turbopack build succeeds; TypeScript checks pass; ESLint fails (170 warnings on legacy `any` types) | None |
...
| **DOCKER-002** | Containers use service names instead of localhost | P0 | ✅ **PASS** | Docker-compose.yml routes variables via composition hosts | None |
...
| **AUTH-001** | Register, login, profile, refresh, logout lifecycle | P0 | ✅ **PASS** | Admin login through Gateway on port 5200 returns valid JWT | None |
| **RBAC-001** | Role authorization matrix | P0 | ✅ **PASS** | Anonymous rejected with 401; admin Bearer token returns 200 | None |
| **GW-001** | Gateway routes all registered endpoints | P0 | ✅ **PASS** | `/identity/auth/login` and `/health/services` route successfully | None |

*Các test cases còn lại đang để trạng thái **NOT RUN** và sẽ được kiểm thử ở các vòng tiếp theo hoặc UAT thực tế trên môi trường staging.*
