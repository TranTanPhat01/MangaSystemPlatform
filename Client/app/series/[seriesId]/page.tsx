...
export default function SeriesDetailPage() {
  ...
  if (state === 'error') {
    return (
      <main>
        <p role="alert">Unable to load series.</p>
  ...
  return (
    <main>
      <button onClick={() => router.push('/series')}>Back</button>
      <h1>{series?.title}</h1>
      <input
      ...
      />
      <button onClick={() => void create()}>Create chapter</button>
      {chapters.length === 0 ? (
        <p>No chapters.</p>
      ) : (
        <ul>
          {chapters.map((c) => (
            <li key={c.id}>
              <button onClick={() => router.push(`/series/${seriesId}/chapters/${c.id}`)}>
