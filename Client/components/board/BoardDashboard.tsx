...
export function BoardDashboard() {
 ...
 useEffect(() => { void board.loadSeriesInsights(selectedSeriesId); }, [board.loadSeriesInsights, selectedSeriesId]);
 ...
 const canManageIssues = roles.some(role => ['editorialboard', 'admin'].includes(role.toLowerCase()));
 ...
 const schedules = <BoardBottomRow schedules={board.schedules} issues={board.issues} onCreate={board.createSchedule} onPublish={board.publishSchedule} publishingScheduleId={board.publishingScheduleId} isSubmitting={board.isScheduling}/>;
 const cancellation = <CancellationRiskPanel series={board.series} selectedSeriesId={selectedSeriesId} warnings={board.cancellationWarnings} rankingHistory={board.rankingHistory} isLoading={board.isSeriesActionLoading} onSeriesChange={setSelectedSeriesId} onStatusChange={action => board.setSeriesStatus(selectedSeriesId, action)} />;
 const content = activeNav === 'Issue Management' ? <IssueManagement issues={board.issues} canManage={canManageIssues} onCreate={board.createIssue} onStatusChange={(issueId, status) => board.updateIssueStatus(issueId, { status })} onRefresh={board.fetchBoardData} /> : activeNav === 'Board Voting' ? voting : activeNav === 'Rankings' ? rankings : activeNav === 'Cancellation Review' || activeNav === 'Decision History' ? cancellation : activeNav === 'Publication Schedule' ? schedules : <div className="space-y-6">{voting}{rankings}{cancellation}{schedules}</div>;
